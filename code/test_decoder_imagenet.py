...
dataset_path = os.path.join(basepath, "../datasets/dataset_encoder_imagenet_rescaled.h5")
...
for i in range(0, 20):
    encoding = encoder.predict(X_train[i:i+1, :, :])
    print("Encoding")
    print(encoding.shape)
    x = ((X_train[i:i+1, :, :])*255).squeeze().astype(np.uint8)
    y = ((decoder.predict(encoding[0:1, :, :, :]))*255).squeeze().astype(np.uint8)
    res = np.concatenate((x, y), axis=1)
    print("Model prediction")
    classes = classifier.predict(X_train[i:i+1, :, :]).squeeze()
    print(classes)
