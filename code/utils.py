...
def make_decoder_model():
    ...
    model.add(layers.Conv2D(filters=512, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
    model.add(layers.Conv2D(filters=512, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
        # model.add(layers.Conv2D(filters=512, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
    ...
    model.add(layers.BatchNormalization())
    model.add(layers.Conv2D(filters=256, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
    model.add(layers.Conv2D(filters=256, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
        # model.add(layers.Conv2D(filters=256, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
    # Fourth block (pool->conv->conv)
    model.add(layers.Conv2DTranspose(filters=128, kernel_size=2, padding="same", strides=2))
    model.add(layers.BatchNormalization())
    model.add(layers.Conv2D(filters=128, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
        # model.add(layers.Conv2D(filters=128, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
    # Fifth block (pool->conv->conv)
    model.add(layers.Conv2DTranspose(filters=64, kernel_size=2, padding="same", strides=2))
    model.add(layers.BatchNormalization())
    model.add(layers.Conv2D(filters=64, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
        # model.add(layers.Conv2D(filters=64, padding="same", kernel_size=3, activation=layers.LeakyReLU(0.2)))
    # Output (conv)
